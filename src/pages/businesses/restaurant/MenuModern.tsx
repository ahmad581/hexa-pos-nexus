  ...
  if (!selectedCategory) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground mb-2">{t('menu.categories')}</h2>
          <p className="text-muted-foreground">{t('menu.selectCategory')}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {categories.filter(cat => cat.value !== 'all').map((category) => {
            ...
            return (
              <Card 
                key={category.value} 
                className="bg-card border-border hover:bg-accent cursor-pointer transition-colors p-6"
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{itemCount} {t('menu.items')}</p>
                </div>
              </Card>
            );
          })}
          ...
          >
              <div className="text-center">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-1">{t('menu.allItems')}</h3>
                <p className="text-sm text-muted-foreground">{menuItems.length} {t('menu.items')}</p>
              </div>
          </Card>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">{selectedCategoryLabel}</h2>
          <p className="text-muted-foreground">{filteredItems.length} {t('menu.itemsAvailable')}</p>
        </div>
      </div>
            ...
            <div className="p-6">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className="flex-1 bg-primary hover:bg-primary/90 disabled:bg-secondary"
                >
