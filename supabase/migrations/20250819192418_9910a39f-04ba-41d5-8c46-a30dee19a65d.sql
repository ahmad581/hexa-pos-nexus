...
BEGIN
  ...
  work_date := DATE(COALESCE(NEW.check_in_time, OLD.check_in_time));
  ...
  WHERE employee_id = COALESCE(NEW.employee_id, OLD.employee_id)
    AND DATE(check_in_time) = work_date;
...
CREATE INDEX idx_employee_work_sessions_date ON public.employee_work_sessions(DATE(check_in_time));
