...
The Notification System provides real-time alerts for Managers and SuperManagers about critical business events. Notifications are automatically generated by database triggers and delivered in real-time via Supabase subscriptions.
...
┌─────────────────────────────────────────────────────────────┐
│                    EVENT SOURCES                            │
│   (Loan Requests, Backups, Inventory, Orders, Settings)    │
│                         │                                   │
│                         ▼                                   │
│         ┌─────────────────────────────┐                    │
│         │   Database Triggers         │                    │
...
│          ▼          ▼          ▼                           │
│     ┌─────────┐ ┌────────────────┐ ┌────────────────┐     │
│     │notifica-│ │ notification_  │ │   Real-time    │     │
│     │  tions  │ │  recipients    │ │  Subscription  │     │
│     └─────────┘ └────────────────┘ └────────────────┘     │
│                                           │                │
│                                           ▼                │
│                    ┌──────────────────────────────┐       │
│                    │  Header Bell Icon & Panel   │       │
...
Links notifications to specific users with per-user read status.
...
#### order_audit_log
Tracks order modifications for anomaly detection.
...
order_audit_log (
  ...
  branch_id: TEXT
  business_id: UUID
  action_type: TEXT ('UPDATE', 'DELETE')
  old_data: JSONB
  new_data: JSONB
  performed_by: TEXT
...
| Type | Trigger Event | Severity | Navigation |
|------|---------------|----------|------------|
| loan_request | New loan application | warning | /employees |
| backup_completed | Backup finished | info | /settings |
| backup_failed | Backup error | critical | /settings |
| settings_changed | Settings modified | info | /settings |
| stock_out | Item out of stock | critical | /inventory |
| stock_low | Item below min stock | warning | /inventory |
| stock_expiring | Item expiring within 7 days | warning | /inventory |
| stock_expired | Item expired | critical | /inventory |
| inventory_request | New stock request | info | /inventory |
| order_anomaly | 10+ order modifications in 1 hour | critical | /orders |

### Database Triggers

The following triggers automatically create notifications:

1. **trigger_notify_loan_request**: Fires on INSERT to `employee_loans` with status='pending'
2. **trigger_notify_backup_status**: Fires on INSERT to `backup_history`
3. **trigger_notify_inventory_status**: Fires on UPDATE to `inventory_items` when status changes
4. **trigger_notify_inventory_request**: Fires on INSERT to `inventory_requests`
5. **trigger_log_order_modification**: Logs all order updates/deletes to `order_audit_log`
6. **trigger_check_order_anomaly**: Detects excessive order modifications (10+ in 1 hour)

### Database Functions

#### is_manager_for_business(user_id, business_id)
Checks if a user is a Manager or SuperManager for a given business.

```sql
is_manager_for_business(_user_id UUID, _business_id UUID) → BOOLEAN
```

#### get_business_managers(business_id)
Returns all manager user_ids for a business.

```sql
get_business_managers(_business_id UUID) → TABLE(user_id UUID)
```

#### create_notification(...)
Creates a notification and assigns it to all relevant managers.
...
} = useNotifications({ limit: 50, autoRefresh: true });
```

### Real-time Subscription

The system uses Supabase Realtime to push new notifications instantly:
...
#### NotificationPanel
Popover dropdown in the header showing notification list:
- Unread count badge on bell icon
- List of recent notifications grouped by time
...
- SELECT: Managers can view notifications for their business
- INSERT: Only system/triggers can insert (via SECURITY DEFINER function)
- DELETE: SuperManagers can delete old notifications

**RLS Policies for notification_recipients**:
- SELECT: Users can view their own recipient records
- UPDATE: Users can mark their own notifications as read

### Setup Notes

1. **Realtime Enabled**: The `notification_recipients` table has realtime enabled for instant updates
2. **Manager Detection**: Uses `is_manager_for_business()` function to find notification recipients
3. **Business Isolation**: Notifications are scoped to business_id for multi-tenant security
...
| 1.6 | January 2025 | Added Notification System with real-time alerts for managers, database triggers for loan/backup/inventory/order events, and order anomaly detection |
